<template>
	<view class="goods-details">
		...
		<!-- #endif -->
		<u-loading mode="circle" :show="isFirstLoading"></u-loading>
		<!-- <loading-view v-if="isFirstLoading"></loading-view> -->
		<view class="contain" v-if="!isNull">
			<!-- <bubble-tips top="40rpx"></bubble-tips> -->
			<!-- <u-swiper :list="productDetail.images" indicator circular height="750rpx"/> -->
			<product-swiper :imgUrls="productDetail.images" :video="productDetail.video"></product-swiper>
			<!-- 秒杀 -->
			<view class="seckill row-between" v-if="productDetail.activityType == ProductActivityTypeEnum.SECKILL">
					...
					<view class="row white info">
						<view style="align-items: baseline;" class="row ml20">
							...
							<price-format :subscript-size="32" :first-size="46" :second-size="32"
								:price="checkedProduct.activityPrice" :weight="500"></price-format>
							<view class="ml10">
								<price-format :subscript-size="30" :line-through="true" :first-size="30"
									:second-size="30" :price="checkedProduct.price">
								</price-format>
							</view>
						</view>
					</view>
			...
			<!-- 拼团 -->
			<view class="group" v-if="productDetail.activityType == ProductActivityTypeEnum.GROUP_BARGAIN">
			...
			<view class="goods-info bg-white">
				<view class="info-header row" v-if="productDetail.activityType != ProductActivityTypeEnum.SECKILL">
					<view class="price row flex1">
						<view class="primary mr10">
							<price-format :first-size="46" :second-size="32" :subscript-size="32"
								:price="checkedProduct.price" :weight="500"></price-format>
						</view>
						<!-- <view class="line-through muted md">
							<price-format :price="productDetail.market_price"></price-format>
						</view> -->
					</view>
					<image class="icon-share" src="/static/images/icon_share.png" @tap="showShareBtn = true"></image>
				</view>
				<view class="row"
					v-if="productDetail.activityType == ProductActivityTypeEnum.NORMAL && checkedProduct.memberPrice">
					<view class="vip-price row">
						<view class="price-name xxs">会员价</view>
						<view style="padding: 0 11rpx">
							<price-format :price="checkedProduct.memberPrice " :first-size="26" :second-size="26"
				...
				<view class="row">
					<view class="name lg bold">{{ productDetail.skuName }}</view>
					<!-- 秒杀显示分享按钮 -->
					<image class="icon-share" src="/static/images/icon_share.png" @tap="showShareBtn = true"
						v-if="productDetail.activityType == ProductActivityTypeEnum.SECKILL"></image>
				</view>
				<view class="row-between xs lighter" style="padding: 0 24rpx 20rpx">
					<text>库存: {{ checkedProduct.stock }}件</text>
					<text>销量: {{ productDetail.totalSaleCount }}件</text>
					<text>浏览量: {{ productDetail.totalClickCount }}次</text>
				</view>
			</view>
			<view class="group-play bg-white mt20"
				v-if="productDetail.activityType == ProductActivityTypeEnum.GROUP_BARGAIN">
			...
			<!-- 			<view class="discount mt20 bg-white" v-if="couponList.length || productDetail.order_give_integral">
				<view class="row" style="align-items: flex-start;">
					<view class="text muted">优惠</view>
					<view style="flex: 1">
						<view :class="['row coupons', {mb30: productDetail.order_give_integral > 0}]"
						...
						<view class="row integral" style="align-items: flex-start;"
							v-if="productDetail.order_give_integral">
							<view class="flexnone">
								<u-tag text="积分" size="mini" type="primary" mode="plain" />
							</view>
							<view class="ml20">下单最多可获得{{productDetail.order_give_integral}}积分</view>
						</view>
					</view>
				</view>
			</view> -->
			...
			<!-- 销售规格 -->
			<view class="spec row bg-white mt20" @tap="showSaleAttr = true">
				<view class="text lighter">已选</view>
				<view class="line1 mr20" style="flex: 1;">{{ checkedProduct.saleValueStr }}</view>
				<image class="icon-sm" src="/static/images/arrow_right.png"></image>
			</view>
			...
			<!-- 			<view class="evaluation bg-white mt20">
				<navigator hover-class="none" :url="'/pages/all_comments/all_comments?id=' + productDetail.id"
 ...
 -->
			...
			<!-- <view class="details mt20 bg-white">
				...
				<view class="content">
					<u-parse :html="productDetail.content" :lazy-load="true" :show-with-animation="true"></u-parse>
				</view>
			</view>
			<view class="footer row bg-white fixed">
				...
				<button class="btn column-center" hover-class="none" @tap="collectGoodsFun">
					<image class="icon-md"
						:src="productDetail.is_collect == 1 ? '/static/images/icon_collection_s.png' : '/static/images/icon_collection.png'">
					</image>
		...
		<spec-popup :show="showSaleAttr" :productDetail="productDetail" @close="showSaleAttr = false" @buynow="onBuy"
			@addcart="onAddCart" @change="onChangeProduct" @confirm="onConfirm"></spec-popup>
...
import {addCartItem} from '@/api/cart'
	...
	const showShareBtn = ref < boolean > (false) //分享
	const productDetail = ref < any > ({}) //商品数据
	const countTime = ref < number > (0) //秒杀剩余时间
	const showSaleAttr = ref < boolean > (false) //销售规格弹窗
	...
	// const checkedProduct = ref<CheckedProduct>()//选中的sku
	// const saleValueStr = ref<string>()//销售属性
	const isFirstLoading = ref < boolean > (true) //加载中
	const isNull = ref < boolean > (false) //商品是否存在
	let skuId: number
	let spuId: number
	//初始化信息
	const load = async () => {
		productDetail.value = await getProductDetail(skuId);
	...
	//初始化信息
	const init = async () => {
		productDetail.value = await getProductDetail(skuId, spuId);
		isFirstLoading.value = false
		if (productDetail.value.checkedProduct) {
			Object.assign(checkedProduct, productDetail.value.checkedProduct)
			console.log(checkedProduct)
		}
	}
	//销售规格弹窗
	const showSpecFun = () => {
		console.log('showSpecFun')
		showSaleAttr.value = true
	}
	//购买
	const onBuy = () => {

	}
	const onAddCart = async (skuId: number, buyNum: number) => {
		await addCartItem(skuId,buyNum)
	}
	const onConfirm = () => {}
	const onChangeProduct = (skuId: number) => {
		isFirstLoading.value = false
		// checkedProduct.value.saleValueStr = ''
		productDetail.value.skuInfos.some((val: any) => {
			if (val.skuId == skuId) {
				Object.assign(checkedProduct, val)
				// checkedProduct = val
				productDetail.value.checkedProduct = val
				return true
			}
		})
	}
