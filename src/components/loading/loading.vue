<template>
	<view :class="'loading ' + ( props.vertical ? 'loading--vertical' : '' )">
		<view :class="'loading__spinner loading__spinner--' + props.type" :style="{color, width: props.size + 'rpx', height: props.size + 'rpx'}">
			<view v-for="(item, index) in array12" :key="index" v-if="props.type === 'spinner'" class="loading__dot"></view>
		</view>
		<view class="loading__text" :style="{'font-size': props.textSize + 'rpx', color}">
			<slot></slot>
		</view>
	</view>
</template>
